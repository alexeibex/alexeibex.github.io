    ...
    function handleInterrogationInput(input) {
        ...
        if (currentQ.field === "transmit") {
            if (input.toLowerCase().startsWith('y')) {
                ...
                // DIRECT LOCATION SET (Fixes blank tab issue)
                setTimeout(() => {
...
STYLE_SCSS = """---
---
/* TERMINAL AESTHETIC PROTOCOL v7.1 */
...
def main():
    print("--- INSTALLING LEGAL_OS v7.1 (GATEKEEPER FIX) ---")
    ...
    with open('assets/js/terminal.js', 'w') as f:
        f.write(TERMINAL_JS)
    print(" > Neural Engine (Docs & Mailto Fixed) Updated.")

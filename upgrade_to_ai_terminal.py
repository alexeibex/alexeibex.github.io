...
# Contains NLP Logic, Intent Scoring, and Generative Effects.
TERMINAL_JS = """
document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- 2. INTENT DEFINITIONS (NLP) ---
    // Maps natural language keywords to specific data keys.
    const intentMap = {
    ...
    // --- 3. BOOT SEQUENCE ---
    const bootSequence = [
        { text: "> INITIALIZING LEGAL_OS v5.0 (NEURAL EDITION)...", delay: 200, class: "system-msg" },
        { text: "> LOADING NATURAL LANGUAGE PROCESSOR...", delay: 400, class: "system-msg" },
        { text: "> CONNECTING TO NEURAL NET...", delay: 600, class: "system-msg" },
    ...
    function resolveIntent(input) {
        ...
        for (const [intent, keywords] of Object.entries(intentMap)) {
            ...
            tokens.forEach(token => {
                if (keywords.some(k => token.includes(k))) score++;
            });
            if (score > maxScore) {
                maxScore = score;
                bestIntent = intent;
        ...
        return maxScore > 0 ? bestIntent : null;
    }

    // --- AI LOGIC: SIMULATED PROCESSING ---
    function simulateThinking(callback) {
        ...
        const interval = setInterval(() => {
            ...
            if (progress === 100) {
                ...
                setTimeout(() => {
                    line.remove(); // Hide progress bar when done
                    callback();
                }, 300);
            }
        }, 50);
    ...
    cmdInput.addEventListener('keypress', function (e) {
        if (e.key === 'Enter') {
            ...
            cmdInput.value = '';
            cmdInput.disabled = true; // Disable while thinking
            ...
            simulateThinking(() => {
                ...
                if (lowerInput.startsWith("generate") || lowerInput.startsWith("draft")) {
                    ...
                    response = type ? generateDoc(type) : "> ERROR: PLEASE SPECIFY DOCUMENT TYPE (NDA / CEASE).";
                }
                // 2. Check Hardcoded Commands
                    ...
                    response = "AVAILABLE COMMANDS: experience, education, skills, bar, projects, contact, resume.<br>TRY NATURAL LANGUAGE: 'Where did you work?', 'Draft an NDA'.";
                }
                // 3. Check NLP Intents
                else {
                    const intent = resolveIntent(rawInput);
                    if (intent && knowledge[intent]) {
                        response = knowledge[intent];
                    } else {
                        response = `> ERROR: UNABLE_TO_PARSE_QUERY.
                        <br>> SYSTEM HINT: Try keywords like "work", "skills", "school", or "generate nda".`;
                    }
                }
...
STYLE_SCSS = """---
---
/* TERMINAL AESTHETIC PROTOCOL v5.0 */
...
<body>
    ...
    <div id="main-gui">
        ...
        <div id="input-line">
            ...
            <input type="text" id="cmd-input" autocomplete="off" spellcheck="false" placeholder="Ask me anything (e.g. 'Where did you study?', 'Generate NDA')">
        </div>
...
def main():
    print("--- INSTALLING LEGAL_OS v5.0 (NEURAL EDITION) ---")
    ...
    with open('assets/js/terminal.js', 'w') as f:
        f.write(TERMINAL_JS)
    print(" > Neural Engine (NLP) Installed.")
